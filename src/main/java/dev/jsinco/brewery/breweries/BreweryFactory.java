...
public class BreweryFactory {
    ...
    public static Barrel newBarrel(PlacedBreweryStructure placedBreweryStructure, Location signLocation) {
        ...
        if (largeBarrelMatcher.find()) {
            return new Barrel(signLocation, placedBreweryStructure, 27, Registry.BARREL_TYPE.get(Registry.brewerySpacedKey(largeBarrelMatcher.group(1))));
        }
        Matcher smallBarrelMatcher = SMALL_BARREL_RE.matcher(name);
        if (smallBarrelMatcher.find()) {
            return new Barrel(signLocation, placedBreweryStructure, 9, Registry.BARREL_TYPE.get(Registry.brewerySpacedKey(smallBarrelMatcher.group(1))));
        }
