...
public record DrunkStateImpl(int alcohol, int toxins, double walkSpeedSquared, long timestamp,
                             long kickedTimestamp) implements DrunkState {

    public DrunkStateImpl recalculate(long timestamp) {
        if (timestamp < this.timestamp) {
            return new DrunkStateImpl(this.alcohol, this.toxins, this.walkSpeedSquared, this.timestamp, this.kickedTimestamp);
        }
        ...
        return new DrunkStateImpl(Math.max(0, Math.min(alcohol, 100)), Math.max(0, Math.min(toxins, 100)), this.walkSpeedSquared, timestamp, this.kickedTimestamp);
    }
    ...
    public DrunkStateImpl withPassOut(long kickedTimestamp) {
        return new DrunkStateImpl(this.alcohol, this.toxins, this.walkSpeedSquared, this.timestamp, kickedTimestamp);
    }
}
